    ...
    if (req.token.userid && mongoose.Types.ObjectId.isValid(req.token.userid)) {
        ...
        if (userdata && userdata.status == true && userdata.mobileverified == true) {
            ...
            if (offerid && offerid != '' && mongoose.Types.ObjectId.isValid(offerid)) {
                let existingwishlist = await primary.model(constants.MODELS.onlineofferwishlists, onlineofferwishlistModel).findOne({ offerid: mongoose.Types.ObjectId(offerid), userid: mongoose.Types.ObjectId(req.token.userid) }).lean();
                if (existingwishlist == null) {
                    let obj = {
                        offerid: mongoose.Types.ObjectId(offerid),
                        userid: mongoose.Types.ObjectId(req.token.userid),
                    };
                    await primary.model(constants.MODELS.onlineofferwishlists, onlineofferwishlistModel).create(obj);
                    return responseManager.onSuccess("Offer wishlist placed successfully!", 1, res);
                } else {
                    await primary.model(constants.MODELS.onlineofferwishlists, onlineofferwishlistModel).findOneAndRemove({ offerid: mongoose.Types.ObjectId(offerid), userid: mongoose.Types.ObjectId(req.token.userid) }).lean();
