    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            primary.model(constants.MODELS.eventbookingcoupons, eventbookingcouponModel).find({}).then((eventbookingcouponslist) => {
                return responseManager.onSuccess('eventbookingcoupons list!', eventbookingcouponslist, res);
            }).catch((error) => {
                return responseManager.onError(error, res);
            })
        }else{
        ...
        return responseManager.badrequest({ message: 'Invalid token to get eventbooking coupons list, please try again' }, res);
    }
});
router.post('/getone', helper.authenticateToken, async (req, res) => {
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        return responseManager.badrequest({ message: 'Invalid token to get eventbooking coupon data, please try again' }, res);
    }
});
